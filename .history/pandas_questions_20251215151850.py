...
def merge_referendum_and_areas(referendum, regions_and_departments):
    ...
    # --- normalise keys (very important) ---
    ref["Department code"] = ref["Department code"].astype(str).str.strip()
    areas["code_dep"] = areas["code_dep"].astype(str).str.strip()

    # drop Z (DOM-TOM-COM + abroad)
    ref = ref[~ref["Department code"].str.contains("Z", na=False)]

    # --- optional: normalise numeric columns ---
    num_cols = ["Registered", "Abstentions", "Null", "Choice A", "Choice B"]
    for c in num_cols:
        if c in ref.columns:
            ref[c] = pd.to_numeric(ref[c], errors="raise")  # 或 errors="coerce"
